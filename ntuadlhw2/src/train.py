...
def main():
    ...
    if args.dataset_name is not None:
        ...
        raw_datasets_unsplited = load_dataset(extension, data_files=data_files)

        #Split the training dataset to train and valid
        raw_datasets = raw_datasets_unsplited["train"].train_test_split(train_size=0.8, seed=924)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                prediction.extend(decoded_preds)
                reference.extend(decoded_labels)
        result = metric(prediction,reference)
        #result = {k: round(v * 100, 4) for k, v in result.items()}

        #Appending rouge information
        rouge1_r.append(result['rouge-1']['r']*100)
        rouge1_p.append(result['rouge-1']['p']*100)
        rouge1_f.append(result['rouge-1']['f']*100)

        rouge2_r.append(result['rouge-2']['r']*100)
        rouge2_p.append(result['rouge-2']['p']*100)
        rouge2_f.append(result['rouge-2']['f']*100)

        rougel_r.append(result['rouge-l']['r']*100)
        rougel_p.append(result['rouge-l']['p']*100)
        rougel_f.append(result['rouge-l']['f']*100)
